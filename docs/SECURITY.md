...
- **Cooperative tool-call cancellation (STORY-027)** — the server honors the MCP `notifications/cancelled` protocol message. When a host abandons a long-running tool call, the corresponding `CancellationToken` is fired, which: unregisters AXObserver subscriptions (`wait_for_ui_event`), exits polling loops within one cycle (`wait_for_element_state`), removes NSWorkspace observers (`wait_for_app_event`), aborts the current layer and skips remaining layers (`smart_interact`), and sends SIGTERM (then SIGKILL after 1000ms if still running) to the osascript subprocess (`run_applescript`). This is a resource-hygiene measure — without it an abandoned wait could keep AX observers, osascript subprocesses, or Mach ports allocated indefinitely. Server SIGTERM cancels every in-flight call via the same pathway. The audit chain records cancelled `run_applescript` invocations with `execution_outcome: cancelled`.
